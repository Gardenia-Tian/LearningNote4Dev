...
def roofline(filename,TIME, FLOPS, AIHBM, AIL2=None, AIL1=None, LABELS=None, flag='HBM'):
    ...
    # A100
    memRoofs = [('L1', 9661), ('L2', 4645),  ('HBM', 1555)] 
    # memRoofs = [ ('HBM', 1555)] 
    cmpRoofs = [('TensorFP16', 311869.440/1024),('FP32', 19491.84 /1024)]
    ...
    for i in range(len(AIHBM)):
        if flag == 'L1':
            ...
            ax.plot(float(AIL1[i]),float(FLOPS[i]),c=colors[i%10],marker=styles[0],\
                    linestyle='None',ms=markersize,markerfacecolor='none',\
                    markeredgewidth=markerwidth,label=LABELS[i] if LABELS else "unknown")
            ax.text(float(AIL1[i]), float(FLOPS[i])-1e3, f"({float(AIL1[i]):.2f}, {float(FLOPS[i]):.2f})", fontsize=16)
            ax.plot(float(AIL2[i]),float(FLOPS[i]),c=colors[i%10],marker=styles[1],\
                    linestyle='None',ms=markersize,markerfacecolor='none',\
                    markeredgewidth=markerwidth,label=LABELS[i] if LABELS else "unknown")
            ax.text(float(AIL2[i]), float(FLOPS[i]), f"({float(AIL2[i]):.2f}, {float(FLOPS[i]):.2f}, {float(float(FLOPS[i]) / float(AIL1[i])):.2f})", fontsize=16)
            ax.plot(float(AIHBM[i]),float(FLOPS[i]),c=colors[i%10],marker=styles[2],\
                    linestyle='None',ms=markersize,markerfacecolor='none',\
                    markeredgewidth=markerwidth,label=LABELS[i] if LABELS else "unknown")
            ax.text(float(AIHBM[i]), float(FLOPS[i])-1e3, f"({float(AIHBM[i]):.2f}, {float(FLOPS[i]):.2f})", fontsize=16)
    ...
    ax.text(xlim[0]*1.1,(ylim[1] - 1e5)/1.1, TIME, horizontalalignment='left',verticalalignment='top')
